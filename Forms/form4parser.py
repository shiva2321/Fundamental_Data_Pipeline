...
class Form4Parser(SECFormParser):
    """
    Parser for SEC Form 4 - Statement of Changes in Beneficial Ownership

    Form 4 must be filed within two business days of a transaction that
    changes an insider's ownership stake.
    """
    ...
    def parse(self, document_text: str) -> Dict[str, Any]:
        ...
        result = {
            "form_type": "Form 4 - Statement of Changes in Beneficial Ownership",
...
class Form4AParser(Form4Parser):
    """
    Parser for SEC Form 4/A - Amendment to Form 4

    Form 4/A is used to amend a previously filed Form 4.
    """

    def parse(self, document_text: str) -> Dict[str, Any]:
        """Parse Form 4/A document"""
        result = super().parse(document_text)

        root = self.parse_xml_from_text(document_text)
        if root is not None:
            result["form_type"] = "Form 4/A - Amendment to Statement of Changes in Beneficial Ownership"
            result["filing_info"]["date_of_original_submission"] = self.format_date(
                self.get_text(root, 'dateOfOriginalSubmission')
            )
